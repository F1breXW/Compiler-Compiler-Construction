...
我们设计了**两套完全不同的文法**来验证系统的自动生成能力：

#### 文法1：算术表达式 (Arithmetic)
- **特点**：包含左递归、优先级（乘除 > 加减）、括号嵌套。
- **测试用例**：
  - `arithmetic_1.txt`: `id + id * id` (测试优先级)
  - `arithmetic_2.txt`: `( id + id ) * id` (测试括号)
  - `arithmetic_error1.txt`: `id + * id` (负例测试)
- **结果**：
  - 正例：成功生成语法树，优先级处理正确。
  - 负例：准确报告 "Syntax Error at state 8, unexpected '*'"。

#### 文法2：赋值语句 (Assignment)
- **特点**：包含赋值符号 `:=`，结构不同于算术文法。
- **测试用例**：
  - `assignment_1.txt`: `x := id + id`
  - `assignment_2.txt`: `result := id - num`
- **结果**：
  - 系统自动生成了与文法1完全不同的 DFA (7状态 vs 9状态) 和 LALR 分析表 (12状态 vs 18状态)。
  - 证明了系统具备真正的**多文法适配能力**。

### 2. 中间代码生成测试

#### 文法3：命令式语言 (Imperative)
- **特点**：支持变量声明 `int x;` 和算术赋值。
- **测试用例**：
  - `ic_test1_arithmetic.txt`: 包含多条赋值和混合运算。
- **结果**：
  - 成功生成三地址码序列。
  - 符号表正确记录了变量 `x`, `y` 的类型信息。
